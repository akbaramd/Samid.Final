...
namespace Samid.Api.Endpoints.Auth
{
    public class AuthCompleteProfileEndpoint : Endpoint<AuthCompleteProfileRequest, AuthResponse>
    {
        ...
        public AuthCompleteProfileEndpoint(UserManager<User> userManager, IConfiguration configuration, ApplicationDbContext context)
        {
        ...
        public override async Task HandleAsync(AuthCompleteProfileRequest req, CancellationToken ct)
        {
            ...
            var gradeFieldOfStudy = await _context.GradeFieldOfStudies
                .Include(gfs => gfs.GradeOfStudy)
                .Include(gfs => gfs.FieldOfStudy)
                .FirstOrDefaultAsync(gfs => gfs.GradeOfStudyId == req.GradeOfStudyId && 
                                            gfs.FieldOfStudyId == req.FieldOfStudyId, ct);
            ...
            var userAcademicYearExists = await _context.UserAcademicYears
                .AnyAsync(ay => ay.UserId == user.Id && ay.GradeFieldOfStudyId == gradeFieldOfStudy.Id, ct);

            if (!userAcademicYearExists)
            {
                var userAcademicYear = new UserAcademicYear(user.Id, gradeFieldOfStudy.Id, academicYear.Id);
                _context.UserAcademicYears.Add(userAcademicYear);
                await _context.SaveChangesAsync(ct);
            }
        ...
        private string GenerateJwtToken(User user)
        {
            ...
            var jwtToken = JwtBearer.CreateToken(o =>
            {
                ...
                o.User.Claims.Add(new Claim(ClaimTypes.NameIdentifier, user.Id.ToString() ?? throw new InvalidOperationException()));
